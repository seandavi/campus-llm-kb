...
# Evaluation and Monitoring {#sec-eval-monitoring}
...
## Post-Deployment Monitoring: Catching Drift
...
The pharmacovigilance analogy is useful here. A drug approved by the FDA carries a post-market surveillance obligation: the manufacturer must monitor for adverse events that did not appear in the clinical trials. AI/ML-based Software as a Medical Device is now subject to analogous requirements. The FDA's lifecycle-management guidance for AI-enabled device software functions, issued in draft in January 2025, remains a draft as of August 2026, but the agency's direction is clear: ongoing performance monitoring and active surveillance for drift and bias are expected elements of any clinical AI program [@FDA2025-lifecycle]. The finalized Predetermined Change Control Plan guidance already binds the premarket side of the same lifecycle [@FDA2024-pccp]. Flag this as an area to check; the lifecycle guidance may be final by the time you read this.

## KPI Architecture: Three Tiers
...
: KPI taxonomy for AMC AI monitoring. Technical KPIs catch model degradation. Operational KPIs catch workflow and adoption problems. Outcome KPIs answer whether the tool is achieving its purpose. All three tiers are required; monitoring only technical KPIs is the most common failure mode in practice. {#tbl-kpi-taxonomy}

## Stakeholder Feedback as a Monitoring Method
...
%%| fig-cap: "AI deployment lifecycle. All four AMC domains share this structure. The domain-specific differences are in what constitutes a failed gate (not the existence of gates), what drift detection looks like (not the need for it), and which governance body has escalation authority."
flowchart TD
...
Equity monitoring is a legal requirement under the HHS Section 1557 final rule, which creates an affirmative obligation for health systems to identify and mitigate discrimination in their use of clinical decision support [@HHS2024-1557]. Demographic subgroup performance is not an optional component of technical monitoring. It is a compliance obligation.

The FDA PCCP framework requires that any predetermined changes to an AI/ML-based Software as a Medical Device — retraining triggers, performance floor thresholds, the training data update protocol — be specified and approved before they are implemented [@FDA2024-pccp]. This gives the monitoring program a regulatory anchor: the decommissioning criteria and retraining triggers are documented commitments made at deployment time, not decisions improvised when a problem has already become visible.
...
: Domain comparison for AI monitoring. Each domain shares the general monitoring lifecycle but differs in what KPIs matter most, which stakeholders are primary, and what governance body has escalation authority. {#tbl-domain-comparison}
